...
description: Bug回归测试技能。当修复方案设计完成后触发：(1)基于修复方案生成回归测试脚本, (2)编写Bug复现测试, (3)编写边界条件测试, (4)调用741-bugfix-test-review自动评审, (5)执行测试并生成报告。请务必在用户提及Bug回归测试、修复验证、测试脚本开发时使用此技能 ⚠️【强制】完成后必须调用 741-bugfix-test-review，未通过前禁止声称完成。
alwaysApply: false
...
回归测试脚本开发完成后，调用 `741-bugfix-test-review`。

- [ ] 已重新读取本节全部内容
- [ ] 已调用 `741-bugfix-test-review`
- [ ] 已收到评审通过确认（评分 ≥ 95）
