...
| 输入项 | 来源 | 说明 |
|--------|------|------|
| 技术方案 | `frontend/{项目名}-admin-uniapp/issues/FEATURE-DESIGN-{YYMMDD}-{简述}-tech-design.md` | 610阶段输出 |
| 后端Swagger | `backend/{项目名}-app/` | API接口定义 |
| 现有代码 | `frontend/{项目名}-admin-uniapp/src/` | 现有代码基线 |

## 输出

| 输出项 | 位置 | 说明 |
|--------|------|------|
| 功能代码 | `frontend/{项目名}-admin-uniapp/src/` | UniApp代码 |
| 测试代码 | `frontend/{项目名}-admin-uniapp/src/**/*.spec.ts` | 单元测试（与源文件同目录） |
| 开发文档 | `frontend/{项目名}-admin-uniapp/issues/FEATURE-DESIGN-{YYMMDD}-{简述}-admin-uniapp.md` | 开发记录 |
| 评审报告 | `frontend/{项目名}-admin-uniapp/reviews/` | AI评审报告 |
| 更新文档 | `frontend/{项目名}-admin-uniapp/README.md` | 合并后的主文档 |
| 变更记录 | `frontend/{项目名}-admin-uniapp/CHANGELOG.md` | 代码变更历史 |
...
| 提取项 | 来源文件 | 用途 |
|--------|---------|------|
| 移动端方案 | `frontend/{项目名}-admin-uniapp/issues/FEATURE-DESIGN-*-tech-design.md` | 页面结构、平台适配、交互流程 |
| 后端Swagger | `backend/{项目名}-app/` | API接口定义 |
...
调用技能 `631-feature-admin-uniapp-dev-review`，评分 ≥ 95 通过，< 95 按下表修复后重新评审（最多5轮，round ≥ 6 强制退出）:
...
**文件位置**: `frontend/{项目名}-admin-uniapp/issues/FEATURE-DESIGN-{YYMMDD}-{简述}-admin-uniapp.md`
...
**代码位置**: `frontend/{项目名}-admin-uniapp/src/`

**开发文档**: `frontend/{项目名}-admin-uniapp/issues/FEATURE-DESIGN-{YYMMDD}-{简述}-admin-uniapp.md`

**评审报告**: `frontend/{项目名}-admin-uniapp/reviews/REVIEW-CODE-{YYMMDDHHMM}.md`
