...
| 输出项 | 位置 | 说明 |
|--------|------|------|
| 修复代码 | `frontend/{项目名}-guest-web/src/` | 修复后的代码 |
| 回归测试 | `frontend/{项目名}-guest-web/src/**/*.spec.ts` | 回归测试代码（与源文件同目录） |
| 修复文档 | `frontend/{项目名}-guest-web/issues/BUGFIX-DESIGN-{YYMMDD}-{简述}-guest-web.md` | 修复记录 |
...
调用技能 `731-bugfix-guest-web-review`，评分 ≥ 95 通过，< 95 按下表修复后重新评审（最多5轮，round ≥ 6 强制退出）:
...
**文件位置**: `frontend/{项目名}-guest-web/issues/BUGFIX-DESIGN-{YYMMDD}-{简述}-guest-web.md`
...
**修复代码**: `frontend/{项目名}-guest-web/src/`

**修复文档**: `frontend/{项目名}-guest-web/issues/BUGFIX-DESIGN-{YYMMDD}-{简述}-guest-web.md`
