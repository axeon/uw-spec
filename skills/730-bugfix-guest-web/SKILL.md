...
## `710-bugfix-tech-design` → `730-bugfix-guest-web` 衔接协议

`710-bugfix-tech-design` 设计完成后，`730-bugfix-guest-web` 从以下文件提取开发输入：
...
**并行约束**：`730-bugfix-guest-web` 与 `720-bugfix-java-uniweb`/`730-bugfix-admin-web`/`730-bugfix-admin-uniapp`/`730-bugfix-guest-uniapp`/`740-bugfix-test` 天然独立可并行。
...
730-bugfix-guest-web
    ↓
AI修复代码 → AI评审(`731-bugfix-guest-web-review`) → 自动修复 → 测试执行
    ↓
输出: 修复后的代码
    ↓
