...
| 维度 | 检查要点 | 源技能章节 |
|------|---------|-----------|
| TDD实践 | composables/Store/工具函数有测试、覆盖率≥70%、Vitest + Vue Test Utils | `320-admin-web-dev` → 开发步骤 |
| Vue3规范 | `<script setup>`、Composition API、TypeScript类型定义、禁用any | `220-admin-web-design` → references/web-dev-standards.md |
| 状态管理 | Store按模块划分、Actions职责单一、Pinia setup风格 | `320-admin-web-dev` → 开发步骤 |
| 代码质量 | 目录结构、命名规范、类型完整、字段一致性 | `220-admin-web-design` → references/web-dev-standards.md |
| 字段一致性 | 前端字段与后端DTO字段命名一致（camelCase）、表单绑定与API参数一致 | `220-admin-web-design` → Phase 2 Step 4 |
| 性能优化 | 路由懒加载、v-for key、computed缓存 | `320-admin-web-dev` → 开发步骤 |
| 安全性 | 禁止v-html未转义、路由守卫权限控制、Token安全存储 | `320-admin-web-dev` → 开发步骤 |

## 通过标准

| 等级 | 评分 | 条件 |
|------|------|------|
| 通过 | ≥ 95 分 | 无 Critical 问题，Major ≤ 2，测试覆盖率核心业务 ≥ 70%，Vue3 规范 100%，TypeScript 覆盖率 ≥ 90%，无性能问题 |
...
**评分 < 95（不通过）：**
- [ ] 已调用 `320-admin-web-dev` 技能，传入问题清单
- [ ] 修复完成后已重新执行本技能评审
