---
name: 220-web-vue-design
description: Web端原型设计与开发（设计即代码）。当需要基于PRD进行Web端页面原型设计时触发：(1)确认页面清单与角色权限映射，(2)编写项目README.md页面清单，(3)裁剪230-gencode生成的页面代码，(4)配置路由与导航，(5)确保字段命名与数据库/后端一致，(6)输出可运行的原型项目。当用户提及Web原型、页面设计、Vue原型、前端设计时使用此技能。
alwaysApply: false
...
| 前端项目 | `frontend/{项目名}-web/` | 230-init初始化 + 230-gencode生成的代码 |

## 前置条件

| 前置技能 | 说明 |
|---------|------|
| [220-web-vue-init](../220-web-vue-init/SKILL.md) | 前端项目已通过模板初始化 |
| [220-web-vue-gencode](../220-web-vue-gencode/SKILL.md) | api/types/pages已由代码生成器生成 |
...
循环开始 (round=1)
  │
  ├─▶ 调用技能: 221-web-vue-design-review
...
- [Web端原型评审技能](../221-web-vue-design-review/SKILL.md)
