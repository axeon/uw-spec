...
从 `project-info.md` 读取 `project-name`，扫描所有 Admin 系 Web 前端项目：

```bash
PN=$(grep 'project-name' project-info.md | head -1 | awk '{print $2}'); ls -1d frontend/${PN}-admin-web/ frontend/${PN}-saas-web/ frontend/${PN}-mch-web/ frontend/${PN}-root-web/ frontend/${PN}-ops-web/ 2>/dev/null
```
...
**目录查找规则**：
脚本从 `project-info.md` 读取 `project-name`，查找 `frontend/{project-name}-(root|ops|admin|saas|mch)-web` 目录（如 `my-shop-admin-web`、`my-shop-saas-web`），找到的第一个匹配目录即为目标前端项目。
