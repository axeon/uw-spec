...
description: UniApp移动端原型设计与开发（设计即代码）。当需要基于PRD进行移动端页面原型设计时触发：(1)确认页面清单与角色权限映射, (2)编写项目README.md页面清单, (3)裁剪230-gencode生成的页面代码, (4)配置pages.json路由, (5)确保字段命名与数据库/后端一致, (6)输出可运行的原型项目。当用户提及移动端原型、UniApp设计、小程序原型、跨平台设计时使用此技能。适用于guest（消费者）角色 ⚠️【强制】完成后必须调用 221-guest-uniapp-design-review，未通过前禁止声称完成。
alwaysApply: false
...
| 前置技能 | 说明 |
|---------|------|
| [220-guest-uniapp-init](../220-guest-uniapp-init/SKILL.md) | 移动端项目已通过模板初始化 |
| [220-guest-uniapp-gencode](../220-guest-uniapp-gencode/SKILL.md) | api/types/pages已由代码生成器生成 |
...
设计完成后，调用 `221-guest-uniapp-design-review`。

- [ ] 已重新读取本节全部内容
- [ ] 已调用 `221-guest-uniapp-design-review`
- [ ] 已收到评审通过确认（评分 ≥ 95）
...
```
$PROJECT_ROOT/frontend/{项目名}-guest-uniapp/
...
- [移动端原型评审技能](../221-guest-uniapp-design-review/SKILL.md)
