...
| 维度 | 检查要点 | 源技能章节 |
|------|---------|-----------|
| 需求符合度 | 功能覆盖、业务流程完整 | `220-guest-uniapp-design` → Phase 0 需求确认 |
| 移动端体验 | 触摸操作、手势支持、适配 | `220-guest-uniapp-design` → references/md-design-spec.md |
| 视觉设计 | 风格统一、字体合适、间距合理 | `220-guest-uniapp-design` → references/md-design-spec.md |
| 技术可行性 | 跨平台兼容、性能优化 | `220-guest-uniapp-design` → Phase 2 Step 5 |
| 一致性 | 组件复用、交互统一、字段一致 | `220-guest-uniapp-design` → Phase 2 Step 4 |
| **源技能约定** | 页面路径角色化、pages.json配置、导入路径修正 | `220-guest-uniapp-design` → 架构约定 + Phase 2 |
...
| 等级 | 评分 | 条件 |
|------|------|------|
| 通过 | ≥ 95 分 | 无 Critical 问题，Major ≤ 2，需求符合度 ≥ 95%，移动端体验达标，技术可行 |
...
**评分 < 95（不通过）→ 自动修复循环：**
1. 立即调用 `220-guest-uniapp-design`，传入问题清单
2. 修复完成后立即重新执行本技能评审
