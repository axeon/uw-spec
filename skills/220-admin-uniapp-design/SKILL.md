...
| 前端项目 | `frontend/{项目名}-admin-uniapp/` | 230-init初始化 + 230-gencode生成的代码 |
...
调用技能 `221-admin-uniapp-design-review`，评分 ≥ 95 通过，< 95 按下表修复后重新评审（最多5轮，round ≥ 6 强制退出）:
...
设计（含 PLAN-REVIEW 循环）通过评审后，提示用户进入 **320-admin-uniapp-dev** 进行UniApp移动端业务开发。
