...
| 输入项 | 来源 | 说明 |
|--------|------|------|
| 移动端代码 | `frontend/{项目名}-admin-uniapp/src/` | 开发完成的代码 |
| 测试代码 | `frontend/{项目名}-admin-uniapp/src/**/*.spec.ts` | 组件测试 |
| 覆盖率报告 | `frontend/{项目名}-admin-uniapp/coverage/` | 测试覆盖率数据 |
| 移动端设计 | `frontend/{项目名}-admin-uniapp/README.md` | 220设计阶段产出的设计规范 |

## 输出
| 输出项 | 位置 | 说明 |
|--------|------|------|
| 移动端开发评审报告 | `frontend/{项目名}-admin-uniapp/reviews/REVIEW-CODE-YYMMDDHHMM.md` | 评审结论和问题清单 |
...
**评审对象**: frontend/{项目名}-admin-uniapp/
**参与人员**: @js-lead @system-architect @js-developer
**流转方向**: 通过 -> 进入下一阶段评审；不通过 -> 返回开发修改

## 输出要求
**报告位置**: `frontend/{项目名}-admin-uniapp/reviews/REVIEW-CODE-YYMMDDHHMM.md`
