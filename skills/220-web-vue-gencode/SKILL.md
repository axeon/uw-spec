---
name: 220-web-vue-gencode
description: Web前端代码生成与增量更新。当需要为Vue3 Web前端项目生成或更新代码时触发：(1)首次生成TypeScript接口定义和API调用，(2)库表变动后增量更新代码并生成变更报告，(3)备份旧文件供后续裁剪恢复。
alwaysApply: false
...
扫描 `frontend/` 目录，列出所有可用的前端项目：

```bash
ls -1 frontend/{项目名}-*-web/
```
...
| 没有前端项目 | 报错，提示先执行 220-web-vue-init |
...
|---|--------|----------|----------|-------------|
| 1 | 目标前端项目 | 要为哪个前端项目生成代码？ | 列出所有 {项目名}-{角色}-web 目录 | false |
| 2 | API类型 | 选择API文档对应的类型？( admin:总后台, saas:SAAS管理, mch:SAAS商户, guest:消费者, open:开放接口, custom:自定义地址) | admin, saas, mch, guest, open, custom | false |
...
**目录查找规则**：
脚本会自动在 `frontend/` 目录下查找匹配 `{项目名}-*-web` 格式的目录（如 `my-shop-admin-web`），找到的第一个匹配目录即为目标前端项目。
