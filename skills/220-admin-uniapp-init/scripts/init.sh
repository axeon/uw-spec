...
case "$TEMPLATE_TYPE" in
    saas)
        TEMPLATE_ZIP="${SKILL_DIR}/asserts/saas-uniapp-template.zip"
        ;;
    uniweb)
        TEMPLATE_ZIP="${SKILL_DIR}/asserts/uw-uniapp-template.zip"
        ;;
    *)
...
# 前端项目名格式: {项目名}-{角色}-uniapp
# 角色从命令行参数获取，默认为 guest
FRONTEND_ROLE="${3:-guest}"
FRONTEND_PROJECT_NAME="${PROJECT_NAME}-${FRONTEND_ROLE}-uniapp"
...
if command -v git &> /dev/null; then
    if [ ! -d ".git" ]; then
        git init -q
        git add .
        git commit -m "Initial commit: ${PROJECT_NAME}-uniapp project setup" -q
        echo "  ✓ Git 仓库初始化完成"
    else
        echo "  ✓ Git 仓库已存在"
    fi
