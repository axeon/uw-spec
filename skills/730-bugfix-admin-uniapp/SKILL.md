...
## `710-bugfix-tech-design` → `730-bugfix-admin-uniapp` 衔接协议

`710-bugfix-tech-design` 设计完成后，`730-bugfix-admin-uniapp` 从以下文件提取开发输入：
...
**并行约束**：`730-bugfix-admin-uniapp` 与 `720-bugfix-java-uniweb`/`730-bugfix-admin-web`/`730-bugfix-guest-web`/`730-bugfix-guest-uniapp`/`740-bugfix-test` 天然独立可并行。
...
输入: 修复方案
    ↓
730-bugfix-admin-uniapp
    ↓
AI修复代码 → 平台适配 → AI评审(`731-bugfix-admin-uniapp-review`) → 自动修复 → 测试执行
    ↓
输出: 修复后的代码
    ↓
