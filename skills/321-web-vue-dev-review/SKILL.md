---
name: 321-web-vue-dev-review
description: Web前端Vue3开发评审技能。当Vue3前端代码开发完成后触发：(1)检查TDD测试驱动开发实践，(2)评审Vue3组合式API规范，(3)检查Pinia状态管理设计，(4)评审组件代码质量和复用性，(5)检查前端性能优化和懒加载，(6)检查XSS等安全漏洞防护。请务必在用户提及Vue3代码评审、前端代码检查、Web开发评审、Element Plus评审时使用此技能。
alwaysApply: false
...
| 输入项 | 来源 | 说明 |
|--------|------|------|
| Web前端代码 | `frontend/{项目名}-web/src/` | 开发完成的代码 |
| 测试代码 | `frontend/{项目名}-web/src/**/*.spec.ts` | 单元测试（与源文件同目录） |
| 覆盖率报告 | `frontend/{项目名}-web/coverage/` | 测试覆盖率数据 |
| 前端设计 | `design/frontend/web-design/` | 设计规范参考 |

## 输出
| 输出项 | 位置 | 说明 |
|--------|------|------|
| Web前端开发评审报告 | `frontend/{用户角色}-{终端类型}/reviews/REVIEW-CODE-YYMMDDHHMM.md` | 评审结论和问题清单 |

## 流转关系
```
通过 → 并行进入其他开发评审（311/331）
不通过 → 返回 320-web-vue-dev 修改
```
