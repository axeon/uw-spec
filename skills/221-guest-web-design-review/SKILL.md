...
| README.md | `PROJECT_ROOT/frontend/{项目名}-guest-web/README.md` | 页面清单与路由设计文档 |
| 需求文档 | `PROJECT_ROOT/requirement/prds/*` | 功能需求参考 |

## 输出
| 输出项 | 位置 | 说明 |
|--------|------|------|
| Web端原型评审报告 | `PROJECT_ROOT/frontend/{项目名}-guest-web/reviews/REVIEW-DESIGN-YYMMDDHHMM.md` | 评审结论和问题清单 |
...
| 维度 | 检查要点 | 源技能章节 |
|------|---------|-----------|
| 需求符合度 | 功能覆盖、业务流程完整 | `220-guest-web-design` → Phase 0 需求确认 |
| 用户体验 | 布局合理、操作简洁、反馈明确 | `220-guest-web-design` → references/web-design-spec.md |
| 视觉设计 | 风格统一、配色合理、间距一致 | `220-guest-web-design` → references/web-design-spec.md |
| 技术可行性 | 交互可实现、性能可接受 | `220-guest-web-design` → Phase 2 Step 5 |
| 一致性 | 组件复用、命名规范、状态完整、字段一致 | `220-guest-web-design` → Phase 2 Step 4 |
| **源技能约定** | 页面路径角色化、SFC结构顺序、导入路径修正、路由配置 | `220-guest-web-design` → 架构约定 + Phase 2 |
...
| 等级 | 评分 | 条件 |
|------|------|------|
| 通过 | ≥ 95 分 | 无 Critical 问题，Major ≤ 2，功能覆盖 ≥ 95%，用户体验无重大问题，技术可行 |
...
**评分 < 95（不通过）→ 自动修复循环：**
1. 立即调用 `220-guest-web-design`，传入问题清单
2. 修复完成后立即重新执行本技能评审
