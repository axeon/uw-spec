...
description: 数据库设计技能。当需要设计或优化数据库时触发：(1)设计新系统数据库结构, (2)定义业务实体和数据关系, (3)规划表结构和字段类型, (4)设计索引优化策略, (5)制定数据规范和约束, (6)优化现有数据库设计 ⚠️【强制】完成后必须调用 201-database-design-review，未通过前禁止声称完成。
alwaysApply: false
