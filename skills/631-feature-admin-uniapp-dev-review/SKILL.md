...
| 源技能文件 | 评审时读取的内容 |
|-----------|----------------|
| [630-feature-admin-uniapp-dev/SKILL.md](../630-feature-admin-uniapp-dev/SKILL.md) | **必读全文**：开发规范、完成标准 |

## 输入

| 输入项 | 来源 | 说明 |
|--------|------|------|
| 功能代码 | `PROJECT_ROOT/frontend/{项目名}-admin-uniapp/src/` | 功能开发产出 |
| 测试代码 | `PROJECT_ROOT/frontend/{项目名}-admin-uniapp/src/**/*.spec.ts` | 单元测试 |
| 技术方案 | `PROJECT_ROOT/frontend/{项目名}-admin-uniapp/issues/FEATURE-DESIGN-*-tech-design.md` | 610阶段方案 |
...
评分 < 95 → **不通过**，自动调用 `630-feature-admin-uniapp-dev` 修复，传入评审报告中的问题清单，按 [REVIEW-FIX 循环规范](../0-init/references/review-fix-loop.md) 执行。
