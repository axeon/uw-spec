...
| 原型README | `frontend/{项目名}-guest-uniapp/README.md` | 220设计阶段产出的页面清单和路由设计 |
| 原型页面 | `frontend/{项目名}-guest-uniapp/src/pages/` | 220设计阶段裁剪后的页面代码 |
| 后端Swagger | `backend/{项目名}-app/` 启动后 | API接口定义 |
| API/types | `frontend/{项目名}-guest-uniapp/src/api/`, `src/types/` | 230-gencode生成的代码 |
...
**代码位置**: `frontend/{项目名}-guest-uniapp/`
...
循环开始 (round=1)
  ...
  │    对 frontend/{项目名}-guest-uniapp/ 下代码执行评审
  │
  ├─▶ 获取评审报告: frontend/{项目名}-guest-uniapp/reviews/REVIEW-CODE-YYMMDDHHMM.md
  │
  ├─▶ 判断结果:
  │    ├─ 评分 ≥ 95 → 输出结论，循环结束 ✓
  │    └─ 评分 < 95 → 进入修复步骤
