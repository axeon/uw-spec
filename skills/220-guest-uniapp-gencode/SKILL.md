...
```bash
PN=$(grep 'project-name' project-info.md | head -1 | awk '{print $2}'); ls -1d PROJECT_ROOT/frontend/${PN}-guest-uniapp/
```
...
**目录查找规则**：
脚本从 `project-info.md` 读取 `project-name`，查找 `PROJECT_ROOT/frontend/{project-name}-guest-uniapp` 目录（如 `my-shop-guest-uniapp`），即为目标前端项目。
...
代码生成完成后，提示用户进入 **`220-guest-uniapp-design`** 进行移动端原型设计（设计即代码，裁剪页面 + 配置路由）。

**流程位置**：`220-guest-uniapp-init` → `220-guest-uniapp-gencode` → **`220-guest-uniapp-design`** (+ `221-guest-uniapp-design-review`) → `320-guest-uniapp-dev`
